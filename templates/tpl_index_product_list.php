...
<?php
  ...
  $form = zen_draw_form('filter', zen_href_link(FILENAME_DEFAULT), 'get') . '<div class="grid_layout_select"><label class="gridLabel">'.PRODUCT_LISTING_GRID_SORT_TEXT.'</label>';
?>
...
<?php
  ...
  if (defined('PRODUCT_LISTING_LAYOUT_STYLE_CUSTOMER') and PRODUCT_LISTING_LAYOUT_STYLE_CUSTOMER == '1') {
    echo '' . zen_draw_pull_down_menu('view', array(array('id'=>'rows','text'=>PRODUCT_LISTING_LAYOUT_ROWS),array('id'=>'columns','text'=>PRODUCT_LISTING_LAYOUT_COLUMNS)), (isset($_GET['view']) ? $_GET['view'] : (defined('PRODUCT_LISTING_LAYOUT_STYLE')? PRODUCT_LISTING_LAYOUT_STYLE: 'rows')), 'onchange="this.form.submit()"') . '</div>';  
  }
  // draw alpha sorter
